...
while getopts ':p:hn:' option; do
   ...
   n)
      ...
      avoid_root() {
         ...
         cre_rep=$(aws ${PROFILE} iam generate-credential-report)
         aws ${PROFILE} iam get-credential-report --query 'Content' --output text | base64 -d | cut -d, -f1,5,11,16 >credential_reports.txt
      ...
      mfa_iam() {
         ...
         aws ${PROFILE} iam get-credential-report --query 'Content' --output text | base64 -d | cut -d, -f1,4,8 >mfa_reports.txt
      ...
      days_90() {
         ...
         rep_days=$(aws ${PROFILE} iam get-credential-report --query 'Content' --output text | base64 -d | cut -d, -f1,4,5,9,10,11,14,15,16 | awk -F "," '{print $2}' | sed -n '2p')

         if [ "$rep_days" == "not_supported" ]; then
      ...
      rotated_90() {
         ...
         aws ${PROFILE} iam get-credential-report --query 'Content' --output text | base64 -d >access_key.log
      ...
      uppercase_iam() {
         ...
         up_c=$(aws ${PROFILE} iam get-account-password-policy | grep RequireUpper | awk -F ":" '{print $2}' | sed -e 's/.$//' | sed -e 's/^\s*//')

         if aws ${PROFILE} iam get-account-password-policy | grep "NoSuch" || [ "$up_c" == "false" ]; then
      ...
      lowercase_iam() {
         ...
         low_c=$(aws ${PROFILE} iam get-account-password-policy | grep RequireLower | awk -F ":" '{print $2}' | sed -e 's/.$//' | sed -e 's/^\s*//')

         if aws ${PROFILE} iam get-account-password-policy | grep "NoSuch" || [ "$low_c" == "false" ]; then
            echo -en "${re}WARNING${xx}"
            echo ""
      ...
      sym_c=$(aws ${PROFILE} iam get-account-password-policy | grep RequireSym | awk -F ":" '{print $2}' | sed -e 's/.$//' | sed -e 's/^\s*//' | sed -e 's/^\s*//')

      symbols_req() {

         if aws ${PROFILE} iam get-account-password-policy | grep "NoSuch" || [ "$sym_c" == "false" ]; then
            echo -en "${re}WARNING${xx}"
            echo ""
      ...
      require_num() {
         ...
         num_c=$(aws ${PROFILE} iam get-account-password-policy | grep RequireNumber | awk -F ":" '{print $2}' | sed -e 's/.$//' | sed -e 's/^\s*//')

         if aws ${PROFILE} iam get-account-password-policy | grep "NoSuch" || [ "$num_c" == "false" ]; then
            echo -en "${re}WARNING${xx}"
            echo ""
      ...
      min_len() {
         ...
         min_n=$(aws ${PROFILE} iam get-account-password-policy | grep Minimum | awk -F ":" '{print $2}' | sed -e 's/.$//' | sed -e 's/^\s*//')

         if aws ${PROFILE} iam get-account-password-policy | grep "NoSuch" || [ "$min_n" == "6" ]; then
            echo -en "${re}WARNING${xx}"
            echo ""
      ...
      root_access_key() {
         ...
         aws ${PROFILE} iam generate-credential-report

         aws ${PROFILE} iam get-credential-report --query 'Content' --output text | base64 -d | cut -d, -f1,9,14 | grep -B1 root >root_access_key.log
      ...
      mfa_root() {
         ...
         mfa_en=$(aws ${PROFILE} iam get-account-summary | grep "AccountMFA" | awk -F ":" '{print $2}' | sed -e 's/.$//' | sed -e 's/^\s*//')
      ...
      iam_policies() {
         ...
         iam_users_check=$(aws ${PROFILE} iam list-users | grep Users | awk -F ":" '{print $2}' | sed -e 's/^.//')

         iam_users=$(aws ${PROFILE} iam list-users | grep UserName | awk -F ":" '{print $2}' | sed 's/"//g' | sed -e 's/,//' | xargs)

         attc_pol=$(aws ${PROFILE} iam list-attached-user-policies --user-name $iam_users)
         pol_name=$(aws ${PROFILE} iam list-user-policies --user-name $iam_users)
      ...
      aws_support() {
         ...
         chckarn=$(aws ${PROFILE} iam list-policies --query "Policies[?PolicyName == 'AWSSupportAccess']" | grep Arn | awk -F " " '{print $2}' | sed 's/"//g' | sed 's/,//g')
         chcksup=$(aws ${PROFILE} iam list-entities-for-policy --policy-arn $chckarn | grep Group | awk -F ":" {'print $2'} | sed 's/,//g' | xargs)
      ...
      trail_control() {
         ...
         list=$(aws ${PROFILE} cloudtrail describe-trails | grep trailList | awk -F ":" '{print $2}')
         e_list=" []"

         trail_n=$(aws ${PROFILE} cloudtrail describe-trails | grep Name | grep -v S3 | awk -F ":" '{print $2}' | sed -e 's/^\s*//' -e '/^$/d' | sed -e 's/^"//' | sed -e 's/.$//')

         region_trail=$(aws ${PROFILE} cloudtrail describe-trails | egrep '*IsM*' | tr -s [:space:] | awk -F ":" '{print $2}')
         ...
         fix_trail() {
            ...
            aws ${PROFILE} cloudtrail update-trail --name $trail_n --is-multi-region-trail
         ...
         elif [ "$region_trail" == "$f_trail" ]; then
            echo -e "${gr}OK${xx}"
      ...
      trail_log_control() {
         ...
         log_e=$(aws ${PROFILE} cloudtrail describe-trails | egrep '*LogFile*' | tr -s [:space:] | awk -F ":" '{print $2}' | tr -s \\n)
         ...
         fix_log_control() {
            ...
            aws ${PROFILE} cloudtrail update-trail --name $trail_n --enable-log-file-validation

         }
         if [ "$log_e" == "$t_log" ]; then
            echo -e "${gr}OK${xx}"
            echo "Log file validation is enabled." | tee -a reports/reports.1
         elif [ "$log_e" == "$f_log" ]; then
            echo -e "${yw}INFORMATION${xx}"
      ...
      s3_bucket_log() {
         ...
         ct_bucket=$(aws ${PROFILE} cloudtrail describe-trails --query 'trailList[*].S3BucketName' | grep [0-9A-Z] | sed -e 's/^\s*//' -e '/^$/d' | sed -e 's/^"//' | sed -e 's/.$//')

         echo -en "S3 Bucket: $ct_bucket"

         echo -e ""

         s3_all_users=$(aws ${PROFILE} s3api get-bucket-acl --bucket $ct_bucket --query 'Grants[?Grantee.URI==`http://acs.amazonaws.com/groups/global/AllUsers`]')

         if [ "$s3_all_users" == "[]" ]; then
            echo -e "${gr}OK${xx}"
         ...
         ct_bucket_aut=$(aws ${PROFILE} s3api get-bucket-acl --bucket $ct_bucket --query 'Grants[?Grantee.URI==`http://acs.amazonaws.com/groups/global/AuthenticatedUsers`]')

         if [ "$ct_bucket_aut" == "[]" ]; then
            echo -e "${gr}OK${xx}"
         ...
         s3_bucket_policy=$(aws ${PROFILE} s3api get-bucket-policy --bucket $ct_bucket)

         if [ "$s3_bucket_policy" == "[]" ]; then
            echo -e "${gr}OK${xx}"
      ...
      cloudwatch() {
         ...
         cdw=$(aws ${PROFILE} cloudtrail describe-trails | grep Cloud | sed -e 's/^\s*//' -e '/^$/d' | sed -e 's/^"//' | sed -e 's/.$//' | sed -e 's/.$//')
      ...
      s3_access_log() {
         ...
         bucket_log=$(aws ${PROFILE} s3api get-bucket-logging --bucket $ct_bucket)
      ...
      cmk_kms() {
         ...
         kms_e=$(aws ${PROFILE} cloudtrail describe-trails | grep Kms)
      ...
      cmk() {
         ...
         key_id=$(aws ${PROFILE} kms list-keys | egrep KeyId | awk -F ":" '{print $2}' | sed -e 's/^\s*//' -e '/^$/d' | sed -e 's/^"//' | sed -e 's/.$//')

         kms_l=$(aws ${PROFILE} kms list-keys)

         keys_e=" []"

         fix_key_rotation() {

            aws ${PROFILE} kms enable-key-rotation --key-id $key_id

         }

         if [ "$kms_l" == "$keys_e" ]; then
            echo -e "${re}WARNING${xx}"
            echo "Master Key not found!"
            rotation_e=$(aws ${PROFILE} kms get-key-rotation-status --key-id $key_id | egrep KeyRot | awk -F ":" '{print $2}' | sed -e 's/^\s*//' -e '/^$/d')
         elif [ "$rotation_s" == "false" ]; then
            echo -e "${yw}INFORMATION${xx}"
      ...
      port_22() {
         ...
         bl=$(aws ${PROFILE} ec2 describe-security-groups --filters Name=ip-permission.to-port,Values=22 | grep GroupName | awk -F ":" '{print $2}' | sed -e 's/.$//' | sed -e 's/^\s*//' | sed -e '/^$/d' | sed -e 's/^"//' | sed -e 's/.$//')

         bl_num=$(aws ${PROFILE} ec2 describe-security-groups --filters Name=ip-permission.to-port,Values=22 | grep GroupName | awk -F ":" '{print $2}' | sed -e 's/.$//' | sed -e 's/^\s*//' | sed -e '/^$/d' | sed -e 's/^"//' | sed -e 's/.$//' | cat -n | awk -F " " '{print $1}' | tail -n 1)

         bl_file=$(aws ${PROFILE} ec2 describe-security-groups --filters Name=ip-permission.to-port,Values=22 | grep GroupName | awk -F ":" '{print $2}' | sed -e 's/.$//' | sed -e 's/^\s*//' | sed -e '/^$/d' | sed -e 's/^"//' | sed -e 's/.$//' >allows.log)
      ...
      port_3389() {
         ...
         bl=$(aws ${PROFILE} ec2 describe-security-groups --filters Name=ip-permission.to-port,Values=3389 | grep GroupName | awk -F ":" '{print $2}' | sed -e 's/.$//' | sed -e 's/^\s*//' | sed -e '/^$/d' | sed -e 's/^"//' | sed -e 's/.$//')

         bl_num=$(aws ${PROFILE} ec2 describe-security-groups --filters Name=ip-permission.to-port,Values=3389 | grep GroupName | awk -F ":" '{print $2}' | sed -e 's/.$//' | sed -e 's/^\s*//' | sed -e '/^$/d' | sed -e 's/^"//' | sed -e 's/.$//' | cat -n | awk -F " " '{print $1}' | tail -n 1)

         bl_file=$(aws ${PROFILE} ec2 describe-security-groups --filters Name=ip-permission.to-port,Values=3389 | grep GroupName | awk -F ":" '{print $2}' | sed -e 's/.$//' | sed -e 's/^\s*//' | sed -e '/^$/d' | sed -e 's/^"//' | sed -e 's/.$//' >allows.log)
      ...
      flow_logs() {
         ...
         log_c=$(aws ${PROFILE} ec2 describe-flow-logs | grep FlowLogId | awk -F ":" '{print $2}' | sed -e 's/.$//' | sed -e 's/^\s*//' | sed -e '/^$/d' | sed -e 's/^"//' | sed -e '/^$/d' | sed -e 's/.$//')

         aws ${PROFILE} ec2 describe-flow-logs | grep FlowLogId | awk -F ":" '{print $2}' | sed -e 's/.$//' | sed -e 's/^\s*//' | sed -e '/^$/d' | sed -e 's/^"//' | sed -e '/^$/d' | sed -e 's/.$//' >flow_log.log
      ...
      defsecvpcfunc() {
         ...
         defsecvpc=$(aws ${PROFILE} ec2 describe-security-groups --filters Name=group-name,Values='default' --query 'SecurityGroups[*].{IpPermissions:IpPermissions,IpPermissionsEgress:IpPermissionsEgress,GroupId:GroupId}')
      ...
      unauthapi() {
         ...
         ctrail_gr_name=$(aws ${PROFILE} cloudtrail describe-trails | egrep "*GroupArn" | awk -F ":" '{print $8}')

         metfiln=$(aws ${PROFILE} logs describe-metric-filters --log-group-name CloudTrail/DefaultLogGroup | grep -w UnauthorizedOperation | awk -F ":*" '{print $2}' | awk -F "=" '{print $2}' | awk -F "\\" '{print $2}' | uniq | head -n 1 | sed -e 's/^"//' | sed -e 's/^*//')

         if [ "$metfiln" == "UnauthorizedOperation" ]; then
            echo -e "${gr}OK${xx}"
      ...
      mfametric() {
         ...
         if aws ${PROFILE} logs describe-metric-filters --log-group-name $ctrail_gr_name | grep MFAUsed; then
            echo -e "${gr}OK${xx}"
      ...
      rootmetrfilt() {
         ...
         if aws ${PROFILE} logs describe-metric-filters --log-group-name $ctrail_gr_name | grep userIdentity.invokedBy; then
            echo -e "${gr}OK${xx}"
      ...
      iammetrfilt() {
         ...
         if aws ${PROFILE} logs describe-metric-filters --log-group-name $ctrail_gr_name | grep DeleteGroupPolicy; then
            echo -e "${gr}OK${xx}"
      ...
      ctrametrfilt() {
         ...
         if aws ${PROFILE} logs describe-metric-filters --log-group-name $ctrail_gr_name | grep 'CreateTrail.*UpdateTrail.*DeleteTrail.*StartLogging.*StopLogging'; then
            echo -e "${gr}OK${xx}"
      ...
      conslogmetrfilt() {
         ...
         if aws ${PROFILE} logs describe-metric-filters --log-group-name $ctrail_gr_name | grep grep "Failed authentication"; then
            echo -e "${gr}OK${xx}"
      ...
      cmkmetrfilt() {
         ...
         if aws ${PROFILE} logs describe-metric-filters --log-group-name $ctrail_gr_name | grep "eventSource = kms.amazonaws.com"; then
            echo -e "${gr}OK${xx}"
      ...
      s3metrfilt() {
         ...
         if aws ${PROFILE} logs describe-metric-filters --log-group-name $ctrail_gr_name | grep 's3.amazonaws.com.*PutBucketAcl.*PutBucketPolicy.*PutBucketCors.*PutBucketLifecycle.*PutBucketReplication.*DeleteBucketPolicy.*DeleteBucketCors.*DeleteBucketLifecycle.*DeleteBucketReplication'; then
            echo -e "${gr}OK${xx}"
      ...
      awsconfmetrfilt() {
         ...
         if aws ${PROFILE} logs describe-metric-filters --log-group-name $ctrail_gr_name | grep "eventName=DeleteDeliveryChannel"; then
            echo -e "${gr}OK${xx}"
      ...
      secchametrfilt() {
         ...
         if aws ${PROFILE} logs describe-metric-filters --log-group-name $ctrail_gr_name | grep "AuthorizeSecurityGroupIngress"; then
            echo -e "${gr}OK${xx}"
      ...
      naclmetrfilt() {
         ...
         if aws ${PROFILE} logs describe-metric-filters --log-group-name $ctrail_gr_name | grep "CreateNetworkAcl"; then
            echo -e "${gr}OK${xx}"
      ...
      netgatmetrfilt() {
         ...
         if aws ${PROFILE} logs describe-metric-filters --log-group-name $ctrail_gr_name | grep "CreateCustomerGateway"; then
            echo -e "${gr}OK${xx}"
      ...
      routabchametrfilt() {
         ...
         if aws ${PROFILE} logs describe-metric-filters --log-group-name $ctrail_gr_name | grep "ReplaceRouteTableAssociation"; then
            echo -e "${gr}OK${xx}"
      ...
      vpcchametrfilt() {
         ...
         if aws ${PROFILE} logs describe-metric-filters --log-group-name $ctrail_gr_name | grep "RejectVpcPeeringConnection"; then
            echo -e "${gr}OK${xx}"
...
uppercase_iam() {
   ...
   if aws ${PROFILE} iam get-account-password-policy | grep "NoSuch" || [ "$up_c" == "false" ]; then
      ...
      if [ "$fix_acc" == "y" ]; then
         aws ${PROFILE} iam update-account-password-policy --require-uppercase-characters
      fi
   else
      echo -en "${gr}OK${xx}"
      echo ""
...
lowercase_iam() {
   ...
   if aws ${PROFILE} iam get-account-password-policy | grep "NoSuch" || [ "$low_c" == "false" ]; then
      ...
      if [ "$fix_acc2" == "y" ]; then
         aws ${PROFILE} iam update-account-password-policy --require-lowercase-characters
      fi
   else
      echo -en "${gr}OK${xx}"
      echo ""
...
sym_c=$(aws ${PROFILE} iam get-account-password-policy | grep RequireSym | awk -F ":" '{print $2}' | sed -e 's/.$//' | sed -e 's/^\s*//')
...
symbols_req() {
   ...
   if aws ${PROFILE} iam get-account-password-policy | grep "NoSuch" || [ "$sym_c" == "false" ]; then
      ...
      if [ "$fix_acc3" == "y" ]; then
         aws ${PROFILE} iam update-account-password-policy --require-symbols
      fi
   else
      echo -en "${gr}OK${xx}"
      echo ""
...
require_num() {
   ...
   if aws ${PROFILE} iam get-account-password-policy | grep "NoSuch" || [ "$num_c" == "false" ]; then
      ...
      if [ "$fix_acc4" == "y" ]; then
         aws ${PROFILE} iam update-account-password-policy --require-numbers
      fi
      echo ""
   else
      echo -en "${gr}OK${xx}"
...
min_len() {
   ...
   if aws ${PROFILE} iam get-account-password-policy | grep "NoSuch" || [ "$min_n" == "6" ]; then
      ...
      if [ "$fix_acc5" == "y" ]; then
         aws ${PROFILE} iam update-account-password-policy --minimum-password-length
      fi
   else
      echo -en "${gr}OK${xx}"
      echo ""
...
conslogmetrfilt() {
   ...
   if aws ${PROFILE} logs describe-metric-filters --log-group-name $ctrail_gr_name | grep "Failed authentication"; then
      echo -e "${gr}OK${xx}"
      echo -e "Metric filter for AWS Management Console authentication failures enabled!"
   else
      echo -e "${re}WARNING${xx}"
...
#metric_g=$(aws ${PROFILE} cloudtrail describe-trails | grep CloudWatchLogsLog | awk -F ":" '{print $8}')

#m_filt=$(aws ${PROFILE} logs describe-metric-filters --log-group-name $metric_g)
